  ...
  <RouterLink :to="`/catalog/${product.id}`"
    ...
    <div class="aspect-square bg-surface-muted overflow-hidden relative">
      ...
      <div v-if="product.stock === 0"
        ...
        <span class="text-[10px] font-semibold text-red-700 bg-red-500/20 px-2 py-0.5 rounded-full">Нет</span>
      </div>
      <!-- Последний остаток -->
    ...
    <div class="p-2 flex flex-col gap-1.5">
      ...
      <button v-if="canAdd"
        class="w-full h-7 rounded-lg bg-indigo-500/20 hover:bg-indigo-500 text-indigo-600 hover:text-white transition-all duration-150 flex items-center justify-center active:scale-90"
      ...
      <div v-else class="w-full h-7 rounded-lg bg-surface-muted flex items-center justify-center">
        <span class="text-[10px] text-ink-700">Нет в наличии</span>
      </div>
    </div>
  </RouterLink>
</template>

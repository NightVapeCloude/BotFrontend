  ...
  <RouterLink :to="`/catalog/${product.id}`"
    ...
    <div class="relative aspect-square bg-surface-muted overflow-hidden">
      ...
      <div v-if="product.stock === 0" class="absolute inset-0 bg-surface/70 flex items-center justify-center">
        <span class="badge bg-red-500/20 text-red-700 text-xs">Нет в наличии</span>
      </div>
    </div>
    <div class="p-3 flex flex-col gap-1.5">
      ...
      <div v-if="price" class="flex items-center justify-between mt-auto pt-1">
        <div class="flex items-center gap-1">
          <BynIcon :size="14" class="text-indigo-600 flex-shrink-0" />
          <span class="font-display text-indigo-600 font-semibold text-l">{{ formatPrice(price) }}</span>
        </div>
        <button v-if="product.stock > 0"
          class="w-8 h-8 rounded-xl bg-indigo-500/20 hover:bg-indigo-500 text-indigo-600 hover:text-white transition-all duration-150 flex items-center justify-center active:scale-90"
          @click.prevent="onAdd">

<template>
  <div class="px-4 pt-6 pb-4 space-y-6 animate-fade-in">
    <div>
      ...
      <p class="text-ink-600 text-sm mt-0.5">{{ greeting }}</p>
    </div>
    ...
    <template v-else>
      ...
      <section v-if="news.length">
        ...
        <div class="space-y-3">
          <div v-for="n in news" :key="n.id" class="card p-4">
            ...
            <p class="text-xs text-ink-600 mt-1.5 leading-relaxed">{{ n.body }}</p>
            <p class="text-xs text-ink-400 mt-2">{{ formatDate(n.published_at) }}</p>
          </div>
        </div>
      </section>
          ...
          <div v-for="d in discounts" :key="d.id"
            ...
            <div class="flex items-center gap-3 mb-2">
              ...
              <div class="min-w-0 flex-1">
                ...
                <p class="text-xs text-ink-500 mt-0.5">До {{ formatDate(d.valid_until) }}</p>
              </div>
              <svg class="w-4 h-4 text-ink-400 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M9 18l6-6-6-6"/>
              </svg>
            </div>
            <!-- Условия -->
            <div v-if="hasConditions(d)" class="border-t border-surface-border pt-2 space-y-1">
              <p class="text-[10px] font-semibold text-ink-500 uppercase tracking-wider mb-1">Условия</p>
              <div v-if="d.min_items > 0" class="flex items-center gap-1.5 text-xs text-ink-600">
              ...
              <div v-if="d.requires_group" class="flex items-center gap-1.5 text-xs text-ink-600">
              ...
              <div v-if="d.reg_date_from || d.reg_date_to" class="flex items-center gap-1.5 text-xs text-ink-600">
              ...
              <div v-if="d.is_referral" class="flex items-center gap-1.5 text-xs text-ink-600">
        ...
        <div class="space-y-2">
          <div v-for="a in arrivals" :key="a.id" class="card p-4 flex items-start gap-3">
            ...
            <div class="min-w-0">
              ...
              <p v-if="a.description" class="text-xs text-ink-500 mt-1">{{ a.description }}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <div v-if="!news.length && !discounts.length && !arrivals.length"
        class="text-center py-16 text-ink-400">
